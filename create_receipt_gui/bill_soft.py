...
from footer_box import draw_footer_box
from headers import draw_table_headers
from product_rows import draw_product_rows
from business_box import draw_business_info
from gst_box import draw_gst_box
from to_box import draw_to_box
from summary_row import draw_summary_row
...
def add_product():
    ...
    product_entries.append([sr_no, hsn, name, pack, batch, exp, mrp, qty, f"{int(gst*100)}%", rate, value, amount])
    update_tree()
...
Button(root, text="Generate PDF", command=generate_pdf).pack(pady=10)
