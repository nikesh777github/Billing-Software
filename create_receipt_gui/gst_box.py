...
def draw_gst_box(c, height, invoice_no, today, due_date, status):
    ...
    c.drawString(starting_pixel+5, height - 76, f"Date          : {today}")
    c.drawString(starting_pixel+5, height - 92, f"Due Date  : {due_date}")
    c.drawString(starting_pixel+5, height - 108, f"Status       : {status}")
