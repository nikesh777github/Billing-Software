...
def open_add_data_window(parent_root=None):
    ...
    def refresh_display():
        ...
        if not businesses:
            Label(display_frame, text="No business details found.", fg="gray", bg="#f0f2f5", font=label_font).pack(anchor="center", pady=20)
            return
        ...
        for bid, details in businesses.items():
            ...
            Label(card, text=details.get("Business Name", "Unnamed"), bg="white", font=title_font).pack(anchor="w")

            for key, value in details.items():
                if key != "Business Name":
                    Label(card, text=f"{key}: {value}", bg="white", font=label_font, anchor="w").pack(anchor="w", pady=1)

            # Buttons
            btn_frame = Frame(card, bg="white")
            btn_frame.pack(anchor="e", pady=5)

            Button(btn_frame, text="📝 Edit", command=lambda b=bid, d=details: open_personal_details_form(window, refresh_display, d, b),
